...
import { Package2, ShoppingCart, Store, Zap, X } from "lucide-react"
import { Button } from "@/components/ui/button"

import Image from "next/image"

// Animation sequence states
type AnimationState = "logo" | "mainService" | "service1" | "service2" | "service3" | "complete"
...
export default function IntroAnimation({ onComplete, onSkip }: IntroAnimationProps) {
  const [animationState, setAnimationState] = useState<AnimationState>("logo")
  const timeoutRef = useRef<NodeJS.Timeout | null>(null)

  // Control the animation sequence
  useEffect(() => {
    const sequence = [
      { state: "logo", duration: 2000 },
      // { state: "mainService", duration: 2500 },
      // { state: "service1", duration: 2000 },
      // { state: "service2", duration: 2000 },
      // { state: "service3", duration: 2000 },
      { state: "complete", duration: 500 },
    ]
    ...
    const runSequence = () => {
      ...
      setAnimationState(state as AnimationState)
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gradient-to-b from-blue-50 to-white dark:from-slate-900 dark:to-slate-950 flex items-center justify-center">
      {/* Skip button */}
      <Button
        variant="ghost"
        size="icon"
        className="absolute top-4 right-4 z-50 rounded-full bg-white/80 dark:bg-slate-800/80 shadow-md hover:bg-white dark:hover:bg-slate-800"
        onClick={onSkip}
      >
        <X className="h-4 w-4" />
      ...
      <Button
        variant="link"
        className="absolute bottom-8 text-sm text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300"
        onClick={onSkip}
      >
        Skip to website
      </Button>
...
<g id="Group">
<g id="glyph0-1">
<path id="Vector" d="M89.8125 61.9167L82.7083 90.8958H147.25L138.812 123.812H59.7917L80.8542 39.5208H28.1875L0 152.792H158.042C161.479 152.792 164.896 152 168.583 150.167C172.271 148.312 175.438 145.938 178.583 143.042C181.75 140.417 184.396 137.25 186.75 133.833C188.875 130.396 190.438 126.979 191.5 123.812L198.875 94.3125C199.146 93.25 199.146 91.6875 198.604 90.1042C198.083 88.5208 197.292 86.6667 196.5 84.8333C195.458 83.25 194.667 81.6667 193.875 80.0833C192.812 78.5 192.292 77.4583 191.771 76.3958C192.562 75.875 193.875 74.5625 195.458 72.9792C197.021 71.3958 198.875 69.8125 200.708 67.9792C202.292 66.3958 203.875 64.5417 205.188 62.7083C206.521 61.125 207.312 59.5417 207.833 58.4792L212.562 39.5208H159.896L154.354 61.9167H89.8125ZM196.25 0H38.1875L30.8125 28.9792H215.208C215.729 25.8333 215.729 22.6667 215.208 19.2292C214.688 15.8125 213.354 12.6458 211.792 9.75C210.208 7.125 207.833 4.75 205.188 2.91667C202.562 1.0625 199.396 0 196.25 0Z" fill="#1C1665"/>
</g>
<g id="glyph0-2">
<path id="Vector_2" d="M363.247 90.8958L354.809 123.812H223.101L215.997 152.792H374.038C377.476 152.792 380.893 152 384.58 150.167C388.268 148.312 391.434 145.938 394.58 143.042C397.747 140.417 400.393 137.25 402.747 133.833C404.872 130.396 406.434 126.979 407.497 123.812L415.934 90.8958C416.455 87.7292 416.455 84.5625 415.934 81.1458C415.393 77.7083 414.08 74.5625 412.497 71.6667C410.913 69.0208 408.559 66.6458 405.913 64.8125C403.288 62.9583 400.122 61.9167 396.705 61.9167H291.33L296.851 39.5208H244.184L238.643 61.9167C237.601 65.0833 237.601 68.5 238.122 71.9167C238.643 75.3542 239.955 78.5 241.809 81.1458C243.393 84.0417 245.768 86.4167 248.393 88.25C251.018 90.1042 254.184 90.8958 257.872 90.8958H363.247ZM438.58 0H280.538C277.101 0 273.684 1.0625 269.997 2.91667C266.309 4.75 262.872 7.125 259.726 9.75C256.559 12.6458 253.663 15.8125 251.288 19.2292C248.913 22.6667 247.33 25.8333 246.809 28.9792H431.205L438.58 0Z" fill="#1C1665"/>
</g>
<g id="glyph0-3">
<path id="Vector_3" d="M578.702 152.792L672.493 39.5208H630.348L576.598 104.583L555.785 39.5208H498.368L535.764 152.792H578.702ZM699.618 152.792L727.806 39.5208H675.118L646.931 152.792H699.618ZM467.285 152.792L495.473 39.5208H459.91L431.723 152.792H467.285ZM737.806 0H663.264L639.035 28.9792H730.431L737.806 0ZM462.535 28.9792H552.639L543.41 0H469.91L462.535 28.9792Z" fill="#1C1665"/>
</g>
</g>
<g id="Group_2">
<g id="glyph1-1">
<path id="Vector_4" d="M9.47135 239.208H14.263L23.1172 197.417H41.2422L42.2005 193.021H1.20052L0.221352 197.417H18.3464L9.47135 239.208Z" fill="#1C1665"/>
</g>
</g>
<g id="Group_3">
<g id="glyph1-2">
<path id="Vector_5" d="M33.7297 239.208H38.5214L42.1464 222.062C44.2089 212.5 51.1255 209.312 56.313 209.312C57.7922 209.312 59.2089 209.458 60.6464 209.708L61.6672 204.792C61.0214 204.667 60.2505 204.542 58.8964 204.542C50.938 204.542 46.7297 209.25 44.1464 213.271H44.0214L45.688 205.562H40.9172L33.7297 239.208Z" fill="#1C1665"/>
</g>
<g id="glyph1-3">
<path id="Vector_6" d="M88.8324 239.208H93.624L100.812 205.562H96.0199L92.1449 223.812C90.5199 231.438 85.2282 236.229 75.8324 236.229C69.749 236.229 66.1449 233.896 67.5615 227.25L72.2074 205.562H67.4365L62.9574 226.396C61.2907 234.417 63.4157 240.25 74.6032 240.25C82.0407 240.25 87.9365 236.167 90.124 232.604H90.2699L88.8324 239.208Z" fill="#1C1665"/>
</g>
<g id="glyph1-4">
...
<path id="Vector_27" d="M1029.83 154.06L1013.43 163.835C1012.37 164.459 1011.06 164.459 1010.02 163.835L994.507 154.587C993.105 153.751 991.508 153.311 989.877 153.311H876.748C876.582 153.311 876.422 153.265 876.284 153.179L847.108 135.781C846.318 135.306 846.65 134.092 847.572 134.092H995.932C997.912 134.092 999.858 134.63 1001.56 135.643L1029.83 152.504C1030.42 152.859 1030.42 153.711 1029.83 154.06Z" fill="#ED1C23"/>
        ...
        {animationState === "mainService" && (
          <motion.div
            ...
            exit="exit"
          >
            <div className="bg-white dark:bg-slate-900 p-10 rounded-xl shadow-lg max-w-2xl text-center">
              <h2 className="text-4xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-blue-400">
                Tech-Enabled Distribution & Fulfillment
              </h2>
        ...
        {animationState === "service1" && (
          <motion.div
            ...
            exit="exit"
          >
            <div className="bg-white dark:bg-slate-900 p-10 rounded-xl shadow-lg max-w-md text-center">
              <div className="flex justify-center mb-6">
                <div className="w-20 h-20 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center">
                  <ShoppingCart className="h-10 w-10 text-blue-500" />
                </div>
              </div>
              <h3 className="text-3xl font-bold mb-4">E-Commerce Distribution</h3>
        ...
        {animationState === "service2" && (
          ...
          >
            <div className="bg-white dark:bg-slate-900 p-10 rounded-xl shadow-lg max-w-md text-center">
              ...
              <h3 className="text-3xl font-bold mb-4">Retail & Modern Trade</h3>
              <p className="text-lg text-slate-700 dark:text-slate-300">
        ...
        {animationState === "service3" && (
          ...
          >
            <div className="bg-white dark:bg-slate-900 p-10 rounded-xl shadow-lg max-w-md text-center">
              ...
              <h3 className="text-3xl font-bold mb-4">Quick Commerce Execution</h3>
              <p className="text-lg text-slate-700 dark:text-slate-300">
        ...
        {/* Final transition frame - just fades out */}
      ...
      {/* Background elements */}
      <div className="absolute top-1/4 left-1/4 w-64 h-64 bg-blue-500/10 rounded-full filter blur-[100px] animate-pulse" />
      <div
        className="absolute bottom-1/4 right-1/4 w-72 h-72 bg-blue-400/10 rounded-full filter blur-[100px] animate-pulse"
